...
run_install_cmake() {
	download_cmake 3.31.12
	download_cmake 4.3.4
}
...
download_cmake() {
	...
	wget https://github.com/jkasdbt/AndroidPE-Packages/releases/download/cmake/"$cmake_version".zip --no-verbose --show-progress -N
...
if [ -d "$cmake_dir/3.31.12" ]; then
	log "$cmake_dir/3.31.12 exists. ${BOLD}Deleting cmake..."
	rm -rf "$cmake_dir"
fi

if [ -d "$cmake_dir/4.3.4" ]; then
	log "$cmake_dir/4.3.4 exists. ${BOLD}Deleting cmake..."
	rm -rf "$cmake_dir"
fi
